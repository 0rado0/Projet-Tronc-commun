...
layout(location = 5) in vec3 instance_offset; // instancing position
...
vec3 wind(in vec3 p,in float t) {
	...
	if (p.z/40>limit_wind){
		influence = smoothstep(0.00, (1.00-limit_wind), p.y-limit_wind); // Apply wind only on the last (1-limit_wind)% of the object
	}
...
{   
	...
	vec3 offset = instance_offset; //scaling_grass * vec3( rand(float(gl_InstanceID)-0.5) * 20.0, rand(float(gl_InstanceID) + 1.0) * 20.0 - 6.0, 0.0)
	...
	position.xyz = position.xyz + wind(vertex_position, time); // procedural deformation modeling the wind effect (we scale the deformation along z such that only the tips of the blades are moving while the root remains fixed).
